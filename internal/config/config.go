...
type Config struct {
	...
	// MasterKey — ключ для шифрования API-токенов 3x-ui в БД (AES-256-GCM).
	// Пустой ключ не мешает запуску и просмотру существующих серверов, но новые
	// и заменённые токены нельзя сохранить. Рекомендуется задавать ключ через
	// ${ENV_VAR} в YAML, чтобы секрет не попал в git.
